...
namespace DinoLingo
{
    ...
    {  
        ...
        {  
            ...
            Debug.WriteLine($"Check if file exists ({fileName }) ? at: {folder.Path}");
            try
            {
                ...
                {
                    Debug.WriteLine($"PCLHelper -> IsFileExistAsync -> file exists");
                    return true;
                }
                Debug.WriteLine($"PCLHelper -> IsFileExistAsync ->file DOES NOT exist");
                return false;
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"PCLHelper -> IsFileExistAsync -> Ex: {ex.Message}");
                return false;
            }
        ...
        public async static Task<System.IO.Stream> GetStreamForFile(String fileName, IFolder rootFolder = null) {
            Debug.WriteLine("GetStreamForFile, fileName = " + fileName);
            // get hold of the file system  
            IFolder folder = rootFolder ?? FileSystem.Current.LocalStorage;
